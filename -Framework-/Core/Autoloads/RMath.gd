...
func rad_to_hex(rad):
	var hex = rad * (128/PI)
	return wrapi(hex, 0, 256)

func hex_to_rad(hex):
	var rad = wrapi(hex, 0, 256) * (PI/128)
	return rad

func rad_sin(rad):
	var hex = rad_to_hex(rad)
	return hex_sin(hex)
...
func hex_sin(hex):
	var index = hex % 256
	return SINCOSLIST[index]

func hex_cos(hex):
	var index = (hex + 64) % 256
	return SINCOSLIST[index]

func rad_point_dir(xdist, ydist):
	return hex_to_rad(hex_point_dir(xdist, ydist))

func hex_point_dir(xdist, ydist):
	# Default
	if (xdist == 0) and (ydist == 0):
		return 64
	# Force positive
	var xx = floor(abs(xdist))
	var yy = floor(abs(ydist))
	var angle = 0
	# Get initial angle
	if yy >= xx:
		var compare = xx * 256 / yy
		angle = 64 - ANGLELIST[compare]
	else:
		var compare = yy * 256 / xx
