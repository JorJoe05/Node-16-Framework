...
func get_distance_linear() -> float:
	...
	var point_vec = _ray.target_position
...
func get_distance_vector() -> Vector2:
	if _ignore_collision or not enabled:
		return Vector2.ZERO
	_update()
	return _ray.get_collision_point() - global_position - _compensation()
...
func get_metadata(string:String):
	...
	if collider is TileMap:
		var map_position = collider.local_to_map(_ray.get_collision_point())
...
func _update():
	...
	var global_positions = [
		Vector2(global_position.x-offset, global_position.y+0.5),
		Vector2(global_position.x+0.5, global_position.y-offset),
		Vector2(global_position.x+offset, global_position.y+0.5),
		Vector2(global_position.x+0.5, global_position.y+offset)
	]
	...
	_ray.global_position = global_positions[direction]
	_ray.target_position = target_positions[direction]
